...
export default apiHandler(dailyUpdate);

async function dailyUpdate(req: Request, res: NextApiResponse<any>) {
  if (req.method !== "GET") {
  ...
  for (let i = 0; i < wallets.length; i++) {
    ...
    walletsService.logWalletValue(wallet.id, publicAssetsValue + wallet.cash);
  }

  return res.status(200).json({ message: "Daily log created" });
}
